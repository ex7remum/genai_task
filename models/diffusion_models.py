...
class UnetModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        temb = self.timestep_embedding(t).view(thro.shape[0], -1, 1, 1)
...
# from https://github.com/NVlabs/edm/blob/main/training/networks.py
...
@diffusion_models_registry.add_to_registry(name="DhariwalUnet")
class DhariwalUNet(torch.nn.Module):
    def __init__(self, model_config):
        super().__init__()
        img_resolution, in_channels, out_channels = model_config['img_resolution'], model_config['in_channels'], model_config['out_channels']
        label_dim, augment_dim, model_channels = model_config['label_dim'], model_config['augment_dim'], model_config['model_channels']
        channel_mult, channel_mult_emb, num_blocks = model_config['channel_mult'], model_config['channel_mult_emb'], model_config['num_blocks']
        attn_resolutions, dropout, label_dropout = model_config['attn_resolutions'], model_config['dropout'], model_config['label_dropout']
